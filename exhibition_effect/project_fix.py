...
st.title("專案：跨年份規律性與聲量過濾 A/B 測試 (Multi-Year Pattern & Volume Filter)爆發前三日")
...
# 2. Group B (聲量濾網：從 First Spike 往前推 3 天當天一路到 T-1 的期間)
...
if not df_spike_filtered.empty:
    ...
    # ==============================================================================
    # 💡 [關鍵調整] 考量資訊前導效應，將過濾與計算勝率/報酬率的基準點往前平移 3 天
    # ==============================================================================
    offset_days = 3
    adjusted_start_t = max(-20, first_spike_t - offset_days)

    df_group_b = df_year_trades[
        (df_year_trades["Stock"].isin(target_stocks))
        & (df_year_trades["Relative_Date"] >= adjusted_start_t)
        & (df_year_trades["Relative_Date"] <= -1)
    ].copy()
    ...
    first_spike_label = f"T{int(adjusted_start_t):+d} ~ T-1 (前推{offset_days}天)"
else:
