...
tairos_period_2025 = (
    stocks_price[stocks_price["Date"].between("2025-07-20", "2025-08-28")]
...
t0_idx = tairos_period_2025[tairos_period_2025["Date"] <= start_dt].index[-1]

for company in company_cols:
  prices = tairos_period_2025[company].values
  days = tairos_period_2025["Date"].values
  n = len(prices)
...
pd.DataFrame(all_results).to_csv("tairos_2025_return.csv", encoding="utf-8-sig")
pd.DataFrame(caar_results).to_csv("tairos_2025_accr.csv", encoding="utf-8-sig")